...
const genContent = (fileType, componentName, className, isPage, hasState, functional) =>
  fileType === '.jsx' ?
    functional ? jsxFnFile(componentName, className) : jsxFile(componentName, className, isPage, hasState) :
    functional ? vueFnFile(componentName, className) : vueFile(componentName, className, isPage, hasState);
